    ...
    public static void main(String[] args){
        ...
        try{
            ...
            if(command.equals(Constants.CLI_REGISTRATION_CLIENT)){
                log.info("main [5]: registration client");
                try{
                    String name = args[2];
            ...
            if(command.equals(Constants.CLI_REGISTRATION_RESUME)){
                log.info("main [11]: registration resume");
                try{
            ...
            if(command.equals(Constants.CLI_REGISTRATION_VACANCY)){
                log.info("main [20]: registration vacancy");
                try{
            ...
            if(command.equals(Constants.CLI_HIRE)){
                ...
                try{
                    ...
                    Result result = service.hireEmployee(idResume, idVacancy, test);
                    if(result.getCode() != Constants.CODE_SUCCESS){
                        log.info("main [27]: Работник не нанят, message = {}", result.getMessage());
                    }
            ...
            if(command.equals(Constants.CLI_GIVE_ASSESSMENT)){
                ...
                try{
                    ...
                    Result result = service.giveAssessment(idEmployee, idCompany, quality, description);
                    if(result.getCode() != Constants.CODE_SUCCESS){
                        log.info("main [30]: оценка не дана, message = {}", result.getMessage());
                    }
            ...
            if(command.equals(Constants.CLI_CALCULATE_ASSESSMENT)){
                ...
                try{
                    int idCompany = Integer.parseInt(args[2]);
                    boolean others = Boolean.parseBoolean(args[3]);
                    
                    Result result = service.calculateAssessment(idCompany, others);
                    if(result.getCode() != Constants.CODE_SUCCESS){
                        log.info("main [33]: расчет не произведен, message = {}", result.getMessage());
                    }
                } catch(Exception ex){
